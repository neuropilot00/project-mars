...
- [ ] **지갑 위조/타인 자원 접근 불가**: 대표 경제·함대 mutation 라우트(`/api/fleets`, `/api/transport/start`, `/api/gp/transfer`, `/api/shop/use`, `/api/territory/:id/harvest`) 무인증 차단을 release preflight에 추가. 전체 owner 검증 감사는 계속 필요.
