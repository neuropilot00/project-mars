...
- 조건 부족 카드도 보유 재료를 흐리게 만들지 않는다. 조선소/강화는 `보유`/`부족` 상태 라벨을 직접 보여준다. (v5.82)
...
- 함대지휘 미리보기는 세로 전장 기준이어야 한다.
- 쐐기는 소형함이 선두, 대형함/기함은 후방 중심으로 보여야 한다.
- 전진/후퇴 표기는 함선이 위를 보는 기준에 맞춰 `↑/↓`로 표시한다. (v5.82)

## 5. P5 — Territory Utility

영토는 이미지 등록과 생산/전쟁/캠페인 조건을 연결한다. P5는 임시 기능이 아니라 최종적으로 게임 경제의 바닥재가 되는 풀 시스템이다.

상세 기획 기준 문서: `docs/TERRITORY_UTILITY_PLAN_2026-05-05.md`

### 5.1 Full Target

- 영토는 개인 캔버스, 생산 노드, 전쟁/경제 앵커 역할을 동시에 가진다.
- 영토 생산은 PP, 재료 드롭, 생산 점수, 섹터 프로필, 업그레이드, 역할, 방어 상태를 포함한다.
- 생산 재료는 조선소 제작, 함선 강화, 수리, 마켓 거래, 장기적으로 전쟁 소모품까지 연결한다.
- 영토 업그레이드는 생산/정제/방어/릴레이/아트 가치를 키우는 장기 재료 싱크가 된다.
- 섹터 컨트롤은 영토 면적, 생산력, 방어/함대 주둔, 활동성, 길드 지분을 합산해 장기 전쟁 목표를 만든다.
- 캠페인은 초반에는 영토 확보/이미지/harvest를 가르치고, 후반에는 생산/업그레이드/섹터 영향력을 목표로 사용한다.
- 어드민은 재료 발행량, 소각량, 섹터별 생산량, 의심 harvest, 드롭률/보너스 설정을 볼 수 있어야 한다.

### 5.2 Implementation Order

- P5-1: 영토 생산 가시성.
- P5-2: 재료 harvest와 결과 UI.
- P5-3: 조선소/강화/마켓 재료 연결.
- P5-4: 영토 업그레이드와 역할.
- P5-5: 섹터 컨트롤과 영향력 보너스.
- P5-6: 어드민 경제 튜닝과 악용 방지.

### 5.3 Scope Control

전체 범위는 풀 시스템으로 확정한다. 다만 구현은 `생산 가시성 -> 재료 harvest -> 조선소 연결`을 먼저 안정화한 뒤 업그레이드/역할/섹터 컨트롤/어드민 튜닝으로 확장한다.

초반 캠페인은 희귀 드롭이나 PvP 결과에 막히면 안 된다. common material, production score, 첫 harvest처럼 확정적으로 달성 가능한 목표를 먼저 사용한다.
