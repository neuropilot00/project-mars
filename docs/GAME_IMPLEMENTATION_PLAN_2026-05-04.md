...
- 영토 상세 패널에 생산 요약을 추가한다.
- 기존 PP harvest를 유지하면서 재료 드롭을 추가한다.
- 영토 생산 결과가 조선소 제작/강화 재료 경제로 이어지게 한다.

### 5.2 Later

- 섹터 장악과 자원 생산량 연결.
- 영토 가치가 함선 제작 재료 흐름에 영향을 주게 한다.
- 영토 업그레이드, 영토 역할, 섹터 컨트롤을 단계적으로 붙인다.

### 5.3 Scope Control

첫 구현은 섹터 전쟁 전체가 아니라 `생산 가시성 -> 재료 harvest -> 조선소 연결` 순서로 간다.
섹터 장악/길드 영향력/영토 역할은 생산 루프가 안정된 뒤 붙인다.
