## 2026-07-16 v7.473 — i18n 이중인코딩 대청소 + SEA 사전 배포 누락 수정 (브라우저 실플레이 QA)

브라우저 실플레이 검증(이 세션 최초의 in-browser 육안 QA)에서 발견한 실버그 2건.

[버그1 — MED] i18n.js 대량 이중인코딩(mojibake): 깨진 한글 7,668·이모지 741·â 1,654 시그니처가 사전 파일에
실재 — 브라우저에서 "ð ² SHIP CRATES"/"â ENTER THE CANTINA"/"Fleet Battles Â· Sieges" 로 노출되던 실버그.
- 복구: 라운드트립 스크립트(latin-1 인코딩 가능 구간→utf-8 재해석, 유효+비ASCII 결과일 때만 교체, ASCII/JS구조 불변,
  3패스). i18n.js 2,395 시그니처→0. 잔여 8건은 정상 베트남어(LÃNH THỔ/ÂM THANH) — 올바르게 보존.
- i18n-sea.js: 연속바이트 소실형(lone ð) 잔해 870건은 라운드트립 불가 → tools/sea-i18n.js 에 repair 명령 추가
  (소스 en 의 이모지 토큰 순서 이식, \p{L} 인접 보호로 베트남어 diacritics 안전). + 신규 15문자열(전투 브리프) 번역.
  SEA check 2,398/2,398 100%·토큰 불일치 0.
- 브라우저 실증: 한국어 UI "함대전 · 공성전 · 1:1 GP 결투"/'🎲 함선 가챠'/'베이스 열기' 정상 렌더 확인.

[버그2 — HIGH] assets/i18n-sea.js 가 **한 번도 커밋된 적 없음**: .gitignore `assets/*` 가 신규 루트 소스 파일을
조용히 무시(기존 js 는 규칙 이전 추적 레거시라 계속 추적). v7.446 SEA 배포가 사전 파일 없이 나가 프로덕션에선
404 → id/vi/th 가 통째로 영어 폴백이었다.
- 수정: .gitignore 에 `!assets/*.js` `!assets/*.css` 영구 예외(소스 절대 무시 금지) + i18n-sea.js 최초 커밋.
  백업 html(_backup*)은 계속 무시. 전수 스캔 결과 누락 소스는 i18n-sea.js 단 1건.

[부수] SW 캐시 함정 실측: fix 이전 버전이 mars-v140 에 캐시돼 fix 가 안 보이는 현상 재현 → fix 이후 기준
?v=7485 / sw mars-v141 재범프(교훈: 자산 수정 후엔 반드시 추가 범프).

[검증] node --check(i18n/i18n-sea/sea-i18n툴) PASS, SEA check 100% exit0, 브라우저 육안(로그인 상태 한국어 UI,
이모지/가운뎃점 정상), 베트남어 보호 검증(Đã 오스트립 0, C1 제어문자 0).
...
"음악 생성 툴 없음"을 게임 SFX(WebAudio 절차생성)와 같은 원리의 신디사이즈로 해결 — 다운로드 아님 = 로열티프리.

[신규 도구] tools/gen-bgm.js — 무드별 앰비언트/텐션 루프 합성
- 순수 사인 파셜의 주파수를 loop 길이(24s)의 정수배로 양자화 → 이음매 없는 무한루프(클릭 없음).
- 무드 = 트랙 이름 키워드 결정론 매핑: battle(파워코드+펄스), tension/high(단조+상승펄스), defeat(단조 저역), victory·ending(장조), choice(서스펜디드), ambient(계열별 root/밝기, cv 어둡게·heavy 저역·warm 장3도).
- 이름 해시 시드로 미세 디튠/LFO 변주 → 같은 무드도 트랙별로 다름.

[자산] assets/audio/bgm/*.mp3 — 40트랙(24s, mono, 64k, 총 7.5MB). questId 무관, 씬 bgm 이름과 1:1.

[검증] node --check tools/gen-bgm.js PASS. 전 40트랙 ffprobe(24.0s·mono)·volumedetect(mean −14~−17dB·max −3.5dB=무음 아님·비클리핑)·길이≥20s 확인. 무드별 레벨 차이 존재. git allowlist(v7.467) 로 추적됨.
- 코드 미변경(BGM 플레이어는 v7.466/467 배포) → ?v/sw 범프 불필요. 배포되면 캠페인에서 즉시 재생.

⚠️ 성격: 프로 OST 아님, 기능적 "분위기 베드". 같은 파일명의 프로 트랙으로 언제든 교체 가능(재생성 절차 README).
...
(오너 결정: 배경음 먼저, 라인별 음성은 이후 영어 전용.)

[클라] assets/campaign-system.js — 캠페인 BGM 매니저
- renderCampaignScene 에서 scene.bgm → /assets/audio/bgm/<track>.mp3 를 루프 재생. 같은 트랙이면 안 끊고 유지, 다른 트랙이면 짧게 페이드 전환. closeCampaignStory 에서 페이드아웃 정지.
- 전역 사운드 설정(getNotifSetting('sound'))에 연동 — SFX 와 같은 스위치. bgm 없는 씬은 현재 재생 유지.
- **파일 없거나 자동재생 차단 시 조용히 무음** — 진행에 영향 0(additive). 캠페인은 클릭 진입이라 오디오 제스처 확보됨.

[자산] assets/audio/bgm/README.md — 씬이 참조하는 **40개 트랙 목록**(파일명+사용 씬 수). 로열티프리/CC0 mp3 를 이 폴더에 드롭하면 자동 재생. 음원은 오너가 제공(라이선스 출처 커밋에 남길 것).

[검증] node --check(campaign-system/app-config/sw) PASS. BGM 로직 node 하니스 6케이스 전수 PASS:
첫재생·동일트랙 연속유지(새 Audio 0)·트랙 전환(기존 pause+새 생성)·bgm없는 씬 유지·사운드OFF 무음·stop→pause. sw mars-v135, ?v=7479.

⚠️ 후속: 40개 BGM 음원 파일 드롭 필요(현재는 인프라만, 파일 없어 무음). 라인별 음성(영어 전용)은 별도 단계 — 재생 인프라(voice 필드)는 동일 패턴으로 확장 가능.
...
(battleEngine v7.416). 빠졌던 건 "교전 직전에 매치업을 보고 내 전술 선택을 의식적으로 커밋"하는 순간.

[클라] AI 연습전 교전 직전 전술 브리프 (assets/battle-hub-modal.js)
- challengeAi 에서 함대 선택 후·교전 POST 전에 aiPreBattleBrief() 를 띄운다(§18 gameConfirm).
- 표시: 적 함대(척수/위협/난이도) + 내 함대(척수/기함 유무) + 진형(아이콘·이름·효과/리스크 한줄)
  + 기동 + 편성(프/구/순/전/타 분해) + 교리 경고(기함 없음→지휘보너스 상실 / 경량편중+강적 / 산개→화력↓).
- "진형·기동은 전투 결과에 반영 — 변경은 [함대 지휘]에서" 안내로 능동성 동선을 연결.
- 유저가 백아웃하면 교전 취소. 브리프 자체가 실패하면 try/catch fallthrough 로 기존처럼 바로 교전(전투 차단 안 함).
- 4개국어(tl), §19 무관(인라인 onclick concat 없음), 기존 AI fight 계약·흐름 보존(가산만).

검증(메인): node --check OK, git diff --check 클린. 신규 브리프 로직 node 하니스 실증 —
confirm→교전 진행, 본문에 적이름/진형명/편성 라인 포함, 강건 편성=경고0, 취약 편성(기함X+경량+산개)=경고3,
EN 폴백(편성 F/D/B) 정상. 서버 클린 부팅, battle-hub-modal.js?v=7469 200 서빙. sw mars-v125.
한계: 모달 in-browser 육안 렌더는 로그인 게이트라 정적+하니스 검증(데이터 조립·gameConfirm 계약은 실증).
...
1. 직업 선택 강제 벽 제거 — onboarding STEP 1(직업)에만 skip 버튼이 없어 직업을 고르기 전엔 진행 불가였음
   (실유저 2명이 정확히 step 1에서 멈춤). 직업은 나중에 변경 가능한데도 강제 커밋이라 aha(첫 클레임) 도달 전 이탈.
   → assets/social-live-modal.js renderObStep1에 '나중에 결정' skip 추가. advanceStep(1)은 직업 없이도
   서버가 정상 통과(completeStep step1은 data.job_code 있을 때만 직업 설정).

2. 스타터 GP(설정, 별도 적용) — 신규는 가입 시 0 PP/0 GP라 무료 첫 클레임 뒤 빈손이라 할 게 없었음.
   signup_pp_bonus 0→10(→스타터 100 GP, auth.js:285 경로, 환율10). GP는 환금 불가 싱크 통화라 USDT 페그
   인플레 0(GP→PP/USDT 역방향 없음 확인). ⚠️ 로컬 dev DB에만 적용 — 실서버는 admin Settings에서 동일 설정 필요.

[보류] 온보딩 풀 리오더(aha-first)는 클라(0~4)/서버(0~5) step 매핑 불일치 + 실제 클레임이 오버레이 밖 +
완료 시맨틱 얽힘 + 신규 플로우 런타임 검증 수단 부재(gstack 다운·계정생성 제약)로 블라인드 시 회귀 위험.
검증 환경 확보 후 진행 권장. 이번엔 검증 가능한 저위험 마찰 제거만 적용.

- 검증: node --check social-live-modal.js OK, git diff --check 클린. sw mars-v122, ?v=7464.
  (온보딩 UI 런타임 검증은 툴 부재로 미실시 — 변경은 additive·코드경로 확인됨.)
...
- 키보드 pitchInput을 `(down)-(up)` → `(up)-(down)`로 뒤집음: Down/S = 기수 아래 = 하강. Up/W = 상승.
- 터치 좌스틱도 `+dy` → `-dy`: 스틱 아래로 드래그 = 하강(직관).
- 검증: node --check PASS, Chrome 실구동 — ArrowDown 시 적이 화면 위로 올라감(시점 하강 확인). sw mars-v119, ?v=7461.

## 2026-06-16 v7.428 — ACE 무한 공간 재중심화 (자동 회전 제거)

피드백: "왜 지멋대로 회전?". 원인: v7.426/427의 경계 자동 되돌림(heading을 중심으로 강제 slerp)이 조작권을
빼앗아 함선이 혼자 돌았음. 잘못된 접근(경계를 회전으로 처리).

- 경계 자동 회전/하드캡/카메라 클램프 전부 제거 → 함선은 오직 플레이어 입력으로만 회전.
- recenterWorld(): 플레이어가 원점에서 350 이상 드리프트하면 전체 씬(적/탄/미사일/트레일/폭발/더스트/별/카메라)을
  같은 오프셋만큼 역이동 + 플레이어를 원점으로. 상대 위치 동일 → 시각 점프 0, 사실상 무한 공간이라 벽에 안 닿음.
- WORLD_RADIUS 800→1600(적 far-spawn 1050과 클램프 충돌 방지, 돔 멀리).
- 검증: node --check PASS, Chrome 35초 연속비행(재중심화 다수 발생) — 수평 직진·자동회전 없음·벽/돔/실종 없음·
  점프 아티팩트 없음·무피해·에러0. sw mars-v118, ?v=7460.

## 2026-06-16 v7.427 — ACE 경계 프레이밍 하드닝 (돔-밖 카메라/함선 실종 수정)

피드백: v7.426 후에도 경계에서 함선이 사라지고 와이어프레임 돔만 보임. 원인: 되돌림이 약하고(82%/0.07) 늦어
전진 속도가 이겨 플레이어가 벽까지 기어가고, 추격 카메라가 돔 밖으로 나가 벽을 바깥에서 비춤(함선 프레임 이탈).

- 되돌림 강화·조기화: 0.5R부터 중심 방향 slerp(최대 0.22/frame) → 플레이어가 내부 영역에 머묾.
- 위치 하드캡 0.92R: 가시 경계(돔)에 절대 도달 안 함.
- 카메라 클램프 0.97R: 추격 카메라가 돔을 절대 벗어나지 않음(벽 바깥 프레이밍 차단).
- 검증: node --check PASS, Chrome 24초 연속비행 — 함선 정상 추격 프레임 유지·중심 복귀 선회·돔밖/실종 없음·무피해·에러0.
  sw mars-v117, ?v=7459.

## 2026-06-16 v7.426 — ACE 월드 경계 튕김(앞뒤 진동) 버그 수정

피드백: "별이 고정돼 전진감 없고 화면이 앞뒤로 계속 왔다갔다". 원인: 플레이어가 직진하다 월드 경계(반경 800)에
닿으면 위치는 안으로 밀리지만 heading은 여전히 벽 바깥을 향해 → 매 프레임 벽에 박고 튕기길 반복(에너지 ×1.55
과반사). 벽에 갇혀 제자리 진동 → 별이 안 흐르고 전진감 소멸. 두 증상 같은 원인.

- 경계 튕김 제거: 벽에서 속도를 과반사하는 대신, 경계의 82%부터 heading을 중심 방향으로 부드럽게 slerp(최대 0.07)
  → 벽을 박지 않고 자연스럽게 선회해 아레나로 복귀. 하드 경계에서는 바깥 방향 속도 성분만 제거(튕김 없음).
- 결과: 플레이어가 멈추지 않고 계속 비행 → 더스트/적 흐름 유지(전진감 복원), 앞뒤 진동 소멸.
- 검증: node --check PASS, Chrome 실구동 22초 연속 전진(이전엔 ~20초에 벽 갇힘) — 수평 정상비행·진동 없음·hull 무피해·에러0.
  sw mars-v116, ?v=7458.
- 참고: 직전 "끝없이 선회+발사" 현상은 테스트 자동입력(keydown without keyup) 찌꺼기였고 게임 버그 아님(키 release로 해소).

## 2026-06-16 v7.425 — ACE 적 stand-off 교전 밴드 (회피 가능 난이도)

피드백: "미사일 안 피해지고 중간에 이상함". 원인: v7.424에서 적 속도를 50~76으로 올려 적이 stand-off 없이
코앞(거리<90)까지 돌진 → 점블랭크 사격은 회피 불가, 거대 함선(타이탄 5×)이 시야를 막음.

- stand-off 교전 밴드: 적이 적정 사거리(170+스케일×34)까지 접근 후 멈춰 좌우로 위빙. 너무 가까우면 후퇴(approach=-0.55).
  코앞 돌진/시야 차단/점블랭크 회피불가 제거. 밴드 안에서 strafe 강화(42)로 표적성 낮춤.
- 적 속도 50~76→36~54로 하향(과도한 돌진 완화).
- 적 사격 산포(±0.07) 추가 → 핀포인트 명중 대신 회피 가능한 탄도.
- 검증: node --check PASS, Chrome 실구동 — 적이 적정 거리서 퍼져 위빙, 8초 idle 무피해(이전 ~3초 사망),
  거대함선 시야차단 소멸, 에러0. sw mars-v115, ?v=7457.

## 2026-06-16 v7.424 — ACE 통일 원근 모델 (토미닉3D식 소실점) — 공간/적/공격 전면 정렬

피드백: "막대기 빔/원근감 거의 없음/토미닉3D가 더 입체적". 인정 — 빌보드를 어설프게 쓰며 핵심 깊이를 놓침.
땜질 중단하고 단일 소실점(정면) 기준으로 공간·적·내함선·공격을 한 모델에 맞춰 재설계.

- 빔(공격) 3D 트레이서: 카메라 빌보드 평면 streak(깊이 무관 = 막대기) 폐기. 이제 탄을 월드 공간에서 진행방향으로
  누운 3D 실린더 트레이서로 그려 멀어질수록 가늘어지며 소실점으로 빨려듦(진짜 원근). 트윈 탄은 전방 720(또는
  락온) 수렴점으로 모아 좌우가 중앙으로 수렴하는 사선 형성.
- 발사 위치 교정: 머즐을 함체 뒤(-11)에서 노즈 앞(18+16×함급스케일)으로 이동 — "뒤에서 쏘는" 어설픔 제거. 플라즈마+타이탄 빔 공통.
- 공간 깊이(핵심): 플레이어 주위를 재활용하는 스트리밍 더스트 필드(PERF 280/680점, ±520 큐브, sizeAttenuation).
  전진 시 더스트가 스쳐 지나가며 가까운 건 크게/먼 건 작게 = 전진 원근. 정적 별필드는 배경으로 유지.
- 적(토미닉식 등장): 스폰/wrap을 멀리(620~1050 / 640~1000)+중앙 콘으로 → 소실점 근처 작은 점에서 출발해
  다가오며 폭발적으로 커지고 밖으로 퍼짐. 적 속도 29~44→50~76로 상향(먼 거리 보정).
- 머즐/더스트 사각형 아티팩트 제거: 방사형 글로우 캔버스 텍스처(캐시)를 머즐 플래시·더스트에 적용 → 둥근 글로우.
- 검증: node --check PASS, Chrome 실구동(더스트 스트리밍·적 원거리 등장 후 성장·빔 노즈서 수렴 3D 트레이서·
  머즐 박스 소멸·hull 무피해·에러0). sw mars-v114, ?v=7456.

## 2026-06-16 v7.423 — ACE 전진 슈팅 규율: 뒤로 빠진 적 전방 wrap

문제: 전진형 슈팅인데 한 번 지나친 적이 뒤꽁무니를 영구히 쫓아와 "뒷치기" 느낌. v7.422의 전방 콘 스폰/사격
게이트는 첫 교전·블라인드 저격만 해결, 전진 중 적이 뒤에 누적되는 구조는 그대로였음.

- updateEnemies가 적의 상대 베어링(facing = playerForward·(적-플레이어))을 계산. facing<-0.2 && 거리>280인
  (= 화면 밖 후방으로 멀어진) 적을 플레이어 forward 콘(우±0.7·상±0.35, 거리 330~480)으로 재배치 → 전진하면
  적이 계속 앞에서 옴(뒤꽁무니 견인 소멸). 재배치는 후방 오프스크린에서 일어나 pop-in 없음.
- 사격 게이트도 동일 facing 기준으로 통일(이동 후 위치 재계산). 충돌(램)은 위치 기반 유지.
- 검증: node --check PASS, Chrome 실구동 — 부스트로 초기 무리 지나쳐도 적이 전방에 재등장, 부스트 중 무피해, 에러0.
- sw mars-v113, ?v=7455.

## 2026-06-16 v7.422 — ACE 뒤통수 사격 제거 — 전방 콘 스폰 + 전방 호 사격 게이트

문제: 적이 원점 기준 사방(`randomDirection`)에 스폰돼 플레이어 뒤에 있는 적이 시야 밖에서 미사일을 쏨.
전진형 슈팅에서 블라인드 스팟 저격은 불합리.

- 전방 콘 스폰: makeEnemy가 player 시작 forward(-Z) 콘(x±0.85·y±0.5·z[-1,-0.35])에 스폰 → 적이 앞에서 등장.
- 전방 호 사격 게이트: 적이 플레이어 forward 호(~75°, dot>0.25) 안에 있을 때만 enemyFire. 뒤로 돈 적은
  추격하되 사격 보류 → U턴/브레이크턴으로 마주봐야 쏨. 충돌(램)은 위치 기반이라 유지(블라인드 저격만 차단).
- 검증: node --check PASS, Chrome 실구동 — 적 12기 전방 중앙 클러스터 접근, 시작 시 hull 무피해(뒤통수 소멸), 에러0.
- sw mars-v112, ?v=7454.

## 2026-06-16 v7.421 — ACE 적 함선 함수(nose) 방향 교정

v7.420 빌보드 전환 후 적 함선이 플레이어 반대(함수가 화면 위)를 보는 문제. top-view 아트는 함수가
텍스처 +Y(빌보드 시 화면 위)라, 카메라 빌보드로 띄우면 함수가 플레이어 반대를 향함(택티컬랩도 적을 180° 돌려 씀).

- 적 스프라이트를 면내 180°(`sprite.rotation.z=π`) 회전해 함수가 플레이어를 향하게 함. 그룹 빌보드/뱅킹/틸트
  수학은 불변. Chrome 실구동으로 적이 함수를 플레이어 쪽으로 향한 3/4 각도로 다가오는 것 확인, 에러 0.
- sw mars-v111, ?v=7453.

## 2026-06-16 v7.420 — ACE 적 함선 "종이 팔랑거림" 제거 (빌보드 재설계)

문제: 적 함선이 텍스처가 위를 보도록 바닥에 눕혀진 수평 평면(`sprite.rotation.x=-π/2`)이라, 추격 카메라
(뒤+약간 위)에서 거의 edge-on(선)으로 보였고, 매 프레임 `lookAt(player)` snap이 얕은 각도를 뒤집어
종이 팔랑거림으로 보였다. 함선 아트는 top-view라 수평 평면 자체가 추격뷰와 근본 불일치.

- 적 함선을 카메라 빌보드로 재설계(머즐플래시/볼트가 쓰는 `quaternion.copy(camera.quaternion)` 검증 패턴 재사용):
  top-view 아트가 항상 정면으로 읽히고 절대 edge-on으로 붕괴하지 않음.
- 종이 느낌 방지 "움직임" 설계: ① 고정 3/4 틸트(ENEMY_VIEW_TILT=0.42)로 dead-on 포스터/edge-on 선 둘 다 회피
  → 원근 단축으로 깊이감, ② 횡속도(카메라-right 성분) 기반 뱅킹을 lerp(dt*4)로 적용 → 카드 뒤집힘이 아니라
  기울며 선회하는 전투기로 읽힘, ③ idle 마이크로 롤(bob)로 정지 시에도 굳지 않음.
- 엔진 트레일을 실제 진행방향(-velocity)으로 분사 + 함급 스케일 반영 → 빌보드를 3D 공간에 앵커.
- 충돌/사격은 위치 기반이라 무관(enemyFire는 적→플레이어 방향 사용, 그룹 방향 미의존) — 회귀 없음.
- 검증: <script> node --check PASS, Chrome 실구동 — 적이 3/4 입체 함선으로 뱅킹, edge-on 팔랑거림 소멸,
  플레이어도 boost 시 입체 뱅킹 확인, 콘솔 에러 0. sw mars-v110, ?v=7452.
...
- 모바일/저사양 perf: setPixelRatio 캡(PERF 1.5/데스크탑 2), PERF 플래그(pointer:coarse|폭≤820|touch)로
  별필드 2000→900·폭발 파티클 0.5x(min6)·트레일 축소. 게임플레이/충돌/카메라/조작은 PERF 무관.
- 손맛: 피격 적색 플래시+저주파 톤(셰이크는 기존 position-only 유지·멀미 방지), 락온 확정 펄스+톤,
  히트마커(×)+머즐 플래시+톤, 격침 폭발 강조, 부스트 시 FOV +6deg 이징+스피드라인. WebAudio lazy(제스처 후).
- 검증: node -c OK, Chrome 실구동(에러0·플라즈마 발사·엔진글로우·HUD 정상). sw mars-v106.
- 스코프 밖(다음): FLEET↔ACE 결과반영(밸런스 결정 대기), 3/4후면 전용아트(이미지젠 프롬프트 제공함).
...
프리런칭 저볼륨(D≪1000)에서 rate 가 ceil(20)로 영구 드리프트 → GP 인플레(밴드 의도와 정반대)되는
문제 확인. EVE 정석은 인플레를 환율 캡이 아니라 싱크(full-loss/수리/건조/엣지)로 잡고 환율은 실시장
볼륨이 생긴 뒤 양방향 변동시키는 것. 따라서 볼륨 없는 지금은 밴드를 중립 보류.

- pp_to_gp_dynamic_enabled=false (밴드 스케줄러 no-op, 천장 드리프트 차단), rate=10 base 리셋.
- floor/ceil/step/target/min_activity 보존 — 런칭 후 재튜닝 대상.
- 런칭 활성화 절차를 mig326 주석에 문서화: 24h 환전량 실측 → target=중앙값 → min_activity≈target 20~30%
  → dynamic_enabled=true. 인플레 우려 시 ceil 낮추기보다 싱크 조이기(EVE 정석).
- 검증: psql 적용 확인(enabled=false, rate=10, 밴드설정 보존).
...
- 기존 `/api/shop/*`, `/api/items/instances`, `/api/items/materialize`, `/api/items/dematerialize`, `/api/items/scrolls`, `/api/enhance/*` URL 계약은 유지했다.
...
- `index.html`은 Fleet Command 전용 CSS 링크만 남기고, 공통 모바일 모달 safe-area 규칙은 기존 위치에 유지했다.
- UI/CSS 캐시 반영을 위해 Service Worker 캐시를 `mars-v99`로 갱신했다.

## 2026-06-11 v7.433 — Shipyard 모달 CSS 외부화

- Shipyard 모달, 청사진 카드, 건조큐, 보유함, 강화, 함선 마켓, 모바일 반응형 핵심 스타일을 `assets/shipyard-modal.css`로 분리했다.
- `index.html`은 Shipyard 전용 CSS 링크만 남기고, 공통 모바일 모달 safe-area 규칙은 기존 위치에 유지했다.
- UI/CSS 캐시 반영을 위해 Service Worker 캐시를 `mars-v98`로 갱신했다.

## 2026-06-11 v7.432 — Shipyard 모달 JS 외부화

- Shipyard 상태, 청사진/건조큐/보유함/강화/마켓/상자/조립 렌더와 액션 로직을 `assets/shipyard-modal.js`로 분리했다.
- `index.html`은 Shipyard 모달 마크업과 외부 스크립트 로드만 남기도록 정리했다.
- 기존 전역 함수명, HTML onclick 계약, `MINERAL_*` 전역 참조 순서는 유지했다.
- UI/JS 캐시 반영을 위해 Service Worker 캐시를 `mars-v97`로 갱신했다.
...
- `index.html` 하단의 버그 리포터 인라인 `<style>` 블록을 제거했다.
- 기존 버그 리포터 모바일 좌표와 모달 z-index 계약은 유지했다.
- UI/CSS 캐시 반영을 위해 Service Worker 캐시를 `mars-v93`으로 갱신했다.
...
- 언어 변경/체크인/미션 진행 갱신이 레거시 보드 대신 `loadOpsCommandBoard()`를 호출하도록 정리했다.
- 캠페인 인게임 스토리 렌더러가 에디터 `localStorage` 좌표를 서버 저장 레이아웃보다 우선하던 경로를 제거했다. 에디터 저장값은 서버 API를 통해 인게임에 반영된다.
- 인게임은 마지막 서버 레이아웃 캐시를 첫 프레임에 적용하고, `/api/campaign/editor-layout` 응답으로 캐시/화면을 갱신한다. 오래된 로컬 에디터 좌표로 위치가 어긋나는 문제를 차단한다.
- 함선 강화 확인 모달의 자원/GP 보유 상태를 OK/부족 배경과 테두리로 더 명확히 표시한다.

## 2026-06-10 v7.417 — 내 영토 작전보드 라우팅/완료 상태 보강

- 오늘의 작전 보드에서 재료/아이템 계열 GO가 마켓으로 빠지던 흐름을 `내 아이템` 탭으로 보냈다. 보유 재료/아이템은 `loadBaseInventory()`로 즉시 갱신된다.
- GO 버튼이 BASE 모달/카테고리/서브탭 상태를 직접 맞추도록 보강해 숨겨진 경제/임무 탭도 안정적으로 열린다.
- 오늘의 작전 보드는 `BASE > 내 영토`만 단일 위치로 유지한다. `OPS CONSOLE`은 침공/탐사 발사 콘솔로 남기고 Daily OPS 미러링을 끊었다.
- 작전 완료 표시는 `completed` boolean 외에도 `current >= target`, `progress_pct >= 100`을 인정한다. 완료된 항목에 녹색 표시와 수령 버튼이 누락되는 회귀를 줄였다.
- 서버 Daily OPS 응답과 보상 수령 쿼리도 진행도 기반 완료를 인정한다. stale row가 있어도 수령 시 `completed`가 확정된다.
- UI 캐시 반영을 위해 Service Worker 캐시를 `mars-v86`으로 갱신했다.

## 2026-06-10 v7.416 — 전술 버튼 실제 전투 계산 반영 + 손실가치 리포트

- 서버 전투 엔진의 일반 포격 데미지에 진형/기동 배율을 연결했다. 쐐기/핀서/방어막/구형/라인/선봉방어, 전진/후퇴/측면/산개/재집결이 실제 교환비에 영향을 준다.
- 기함 생존 여부를 전투 품질에 반영했다. 기함이 살아 있으면 지휘 보너스가 유지되고, 기함 격침 후에는 공격/방어 교환이 불리해진다.
- 재집결(`rally`)과 방어막(`screen`)은 수리함 효율을 올려 장기전 선택지로 기능하게 했다.
- 전투 결과 리포트가 `ship_wrecks` 기반 영구 손실 함선 수와 GP 손실가치를 내려준다.
- 패배 분석이 손실 가치 격차를 감지해 고강화/대형함 보호, 상대 저격/폭격 확인, 재건 재료 확보를 추천한다.
- 결과 모달에 `자산 손실` 블록을 추가해 내 함대와 상대의 full-loss 손실 규모를 비교 표시한다.

## 2026-06-10 v7.415 — 함대 지휘 사전 상성 경고/전술 설명 보강

- Fleet Command 모달의 진형/기동 버튼 아래에 현재 선택의 효과와 리스크를 표시했다.
- 함대 미리보기 우측에 `DOCTRINE CHECK`를 추가해 기함 미지정, 지원함 부재, 대형함 호위 부족, 대형함 카운터 부재, 소형 편중, 무보호 전진 리스크를 출격 전 경고한다.
- 전투 결과 리포트에 역할 구도 비교를 추가했다: 태클/전자전/DPS/탱커/저격/폭격/로지 수량을 내 함대와 상대 함대로 나란히 표시한다.
- UI/CSS 캐시 반영을 위해 Service Worker 캐시를 `mars-v85`로 갱신했다.

## 2026-06-09 v7.414 — 함대전 스킬 이벤트/전술랩 연출 보강

- 서버 전투 엔진의 수동 스킬이 실제 `ship_destroyed`/`manual_skill` 이벤트를 남기도록 정리했다.
- 빔포는 기함/대형함 우선, 미사일은 프리깃/저HP 다수 타격, 합체 필살은 대형 목표 광역 타격으로 구분했다.
- 수리함의 `repair_pulse` 이벤트와 수리량 집계를 전투 리포트에 반영했다.
- 기함 격침 데미지가 `ship_destroyed`와 `flagship_destroyed`에서 중복 합산되던 경로를 제거했다.
- 전투 프레임에 함선 `facing`을 포함해 공격 대상 방향을 알 수 있게 했다.
- 전투 결과 모달에 빔포/미사일/EMP/수리 사용 요약을 추가했다.
- 전술랩 화성 배경 밝기를 올리고 수동 빔/미사일 연출 지속시간을 늘렸다.
...
남던 것 개선. MCC 8종을 테두리연결 배경 제외 전 영역 fill_holes → 이진 255 솔리드 → 가우시안 σ0.6
가장자리 AA 로 재처리. 얇은 안테나/날개까지 불투명 유지 + 매끈한 엣지. mcc_frg 대형 확대 검증
(see-through 0, 솔리드). 남은 어두움은 소스 아트 특성(투명 문제 아님).

## 2026-06-05 v7.410 — 전 함선 내부 비침 제거 + 전투 사운드 닫기 버그

#1 함선 내부 반투명(비침): mcc/fsp/cv 22종 전부 — 테두리 연결 배경만 투명, 내부(core)는 완전
불투명(255), 가장자리 2px만 원본 소프트 유지(scipy 라벨/erosion/fill_holes). 청사진 128px 크기에서
국소 반투명이 비치던 것 제거. Chrome 실렌더 검증(see-through 없음, 매끄러운 엣지).

#3 사운드 버그: 사운드 ON 후 전투 뷰어 닫으면 tactical-lab iframe 이 안 죽어 WebAudio/시뮬 루프가
계속 돌아 사운드가 들리던 문제. closeBattleViewer 에서 iframe src=about:blank 으로 완전 언로드
(사운드+rAF+WS 종료) + 결과 fallback 타이머 정리.

## 2026-06-05 v7.409 — MCC 함선 가장자리 부드럽게 (이진 알파 → 게인 커브)

v7.408이 함체를 이진({0,255})으로 채워 가장자리가 거칠게 잘려(안티에일리어싱 X) "이상하게 딴" 느낌이
남던 것 개선. 원본에서 다시: 테두리 연결 배경만 0 + 함체 알파 게인(×2.3, 본체는 불투명까지 끌어올리되
가장자리 그라데이션 유지) + 내부 완전구멍 fill. mcc 8종(bs/crs/dst/ewar/frg/int/snp/titan) 전부
Chrome 실렌더 검증 — 꽉 찬 함체 + 매끄러운 엣지.

## 2026-06-05 v7.408 — MCC 함선 반투명(내부 비침) 수정

MCC 함선 8종 top 스프라이트가 배경제거(rembg/크로마키) 과정에서 함체 내부까지 갉혀 거의 전체가
반투명(mcc_bs: 불투명 4623 vs 반투명 24439)이라 전투에서 배경이 비쳐 유령처럼 보이던 문제.
(FSP/CV는 불투명 정상.)

수정: 테두리에서 연결된 진짜 배경만 투명 유지(scipy 라벨링 flood)하고, 함체(내부 반투명·구멍 포함)는
alpha=255 로 불투명화. mcc_bs/crs/dst/ewar/frg/int/snp/titan 8종. Chrome 실렌더로 before/after 검증
(유령 반투명 → 꽉 찬 함체, 실루엣 보존).
...
원인: tactical-lab 전투 렌더의 스프라이트 게이트(v7.58). 함선 PNG가 'loading' 중이면 벡터 폴백도
안 그려 함선을 전부 투명 처리했다. iframe은 전투를 열 때마다 32개 함선 PNG(~2MB)+화성 텍스처(750KB)를
새로 동시 로드하므로(매번 &t= 캐시버스트), 로딩 윈도우 동안 화면이 비었다. (404면 'failed'→벡터로
보이므로 빈 화면 = 로딩 지연/스톨 경로.)

수정: `_SPRITE_GATE_UNTIL_MS=700` 그레이스 도입. iframe 로드 후 700ms 동안만 벡터를 숨겨(빠른 로드 시
실루엣 깜빡임 방지 — v7.58 의도 유지), 이후엔 PNG 로딩 중이어도 벡터 폴백을 그려 **함선이 항상 보이게**
한다. PNG 로드 완료 시 자동으로 PNG로 교체. 빈 화면 윈도우를 최대 700ms로 한정.

진단 부산물(별도): AI 듀얼은 서버에서 2~6 이벤트(타임라인 ~2프레임)로 즉시 해결 — 시각화는 iframe
로컬 시뮬이 담당(설계대로). fleet-presets?bid 는 참가자 있는 전투엔 실제 편성 반환(battle:54 확인);
참가자 없는 롤백 결번(#27)만 데모 폴백. 검증: 스프라이트 200 서빙 확인, 메인 스크립트 node -c OK.
...
지나야 결과 카드를 표시(짧으면 마지막 프레임 유지). frames<2 즉시종료 케이스도 5초 유지. bvAnimLoop에
_playStartMs 기준 게이트 추가. 일시정지/시크는 그대로 동작.
...
D7/D30 리텐션). GET /api/me/away-briefing(JWT, ship_wrecks+bounty_listings 집계). 프론트:
길드 로드 후 _checkAwayBriefing — hasNews 시 "⚠ 부재 중 피해 보고" 붉은 배너 + "복수하러 가기"
CTA(→PVP 탭). lastLoss 기준 1회만 표시(중복 방지). 엔드포인트/401 검증.

→ 6개 도파민 후보(#1 near-miss, #2 천장게이지, #3 킬메일, #4 승리슬롯, #5 streak, #6 복귀브리핑) + 실드 연출 전부 완료.

## 2026-06-04 v7.393 — 데일리 streak 손실회피 강화 (도파민 #5)

기존 데일리 streak 시스템(daily_logins, 48h 유예)의 시각화 강화: 🔥 연속일수 강조(7일↑ 주황,
14일↑ 🔥×3) + 미체크인 시 "오늘 놓치면 N일 연속 리셋!" 손실회피 경고. 프론트만(데이터 기존).
...
박제(mig312 ship_value_gp/mods). 킬보드: 누적 격파 가치(GP DESTROYED)·최고 격파·행별 가치(💥
고가치 5K+ 강조, +MOD 배지). "내가 N GP짜리 함선 격파" = 격파 쾌감+과시. 백엔드/응답 검증.

## 2026-06-04 v7.390 — 가챠 천장 게이지 (도파민 #2)

크레이트 pity가 텍스트만이던 것을 진행 게이지 바로 시각화(since/pity_pulls 채움). 천장 임박
(잔여≤2)이면 펄스 글로우 + "임박!" 강조 — 매몰비용 "한 번 더" 자극. 기존 pity_remaining 재사용.

## 2026-06-04 v7.389 — 강화 near-miss 연출 (도파민 #1)

함선 강화 실패 중 굴림이 필요 확률을 근소하게(≤8%) 초과한 "아깝게 실패"를 별도 연출: 😖 "단 X%
차이로 실패!" + 붉은 글로우 강화 + 결과 쉐이크 + 사운드. 재도전 욕구↑. 경제중립(연출만).

## 2026-06-03 v7.388 — 함대전 실드(배리어) 연출 + 실드 게이지

함선에 함급별 실드풀 추가(타이탄 0.6×HP ~ 프리깃 0.12×HP). 전투 연출:
- 실드 버블: 미사일/레이저를 막는 청록 배리어 링(잔량 비례 알파). 피격 시 리플 강조.
- 실드 흡수: 로컬 sim에서 실드가 먼저 데미지 흡수(초과분만 선체). 집중사격엔 뚫림(재생<화력).
- 실드 게이지: HP바 위 얇은 청록 바(잔량 표시).
- 천천히 재생. PERF_MODE/소형함은 버블 생략(성능). tactical-lab-v11.html(라이브 뷰어).

## 2026-06-03 v7.387 — 도파민 강화 1차: 공통 보상 연출 + 채굴 잭팟 + 전투보상 juice

팀 분석 기반(경제중립 — 연출/사운드만, 보상량/드롭률 무변):
- rewardBurst() 공통 헬퍼: +N 숫자 팝 + tier별 색/사운드/전설 플래시. reduced-motion 가드, DOM 1개.
- 채굴 레어드롭 차등: tier2(에픽)/tier3(전설 "MOTHERLODE!") 드롭만 잭팟 연출(일반은 조용 — 도파민 둔감 방지). 최고빈도 행동 ROI 1위.
- showRewardToast: 무음·평면이던 전투 보상에 승리/참가 사운드 + GP 숫자 버스트.
가드레일 준수: PP(환금) near-miss 연출 미추가(규제 리스크), 보상량 변경 0(반인플레), 모바일 가드.

## 2026-06-03 v7.386 — 전투 구형 진형도 큰함 안쪽 배치 (미리보기와 일관)

전투(battleEngine) 구형 진형이 랜덤 공전이라 함급 무관이던 것을 수정: orbitDist를 renderRadius
내림차순(큰함 먼저)으로 안쪽부터 채워 큰/중요한 함선=안쪽 코어(기함 옆), 작은 함선=바깥 궤도
(소모성 스크린). wedge/screen/vanguard는 이미 함급 정렬 → 이제 4종 일관. 위치만 영향(밸런스 무관).

## 2026-06-03 v7.385 — 진형 배치 역할 우선순위 (작은함 앞 / 큰함 중앙·기함 옆)

진형 미리보기가 인덱스 순(줄세우기)이라 함선 역할/크기를 무시하던 것을 수정:
- 작고 싼 함선 = 앞/외곽(소모성 스크린), 크고 중요한 함선 = 중앙/기함 옆(보호).
- sphere/pincer/vanguard(안쪽=중앙)는 큰 함선 먼저, wedge/screen/line/echelon(앞=전방)은
  작은 함선 먼저 정렬. 28 캡은 큰(중요) 함선 우선 유지(타이탄/전함이 안 잘림).
검증: wedge=frigate팁→titan기함옆, sphere/vanguard/pincer=titan안쪽→frigate외곽. 스크립트 11/0.
...
updateFleet 클램프(0.55~1.6) + fleetPreviewPoint spread 파라미터 + Fleet Command 밀집도 버튼.
설정/클램프(2.0→1.6) 검증. fresh 체인 완주, 스모크 11/0.
...
표시: 🥉브론즈(1)/🥈실버(10)/🥇골드(50)/🏆에이스(150)/👑레전드(500). 프리뷰 보드 하단 중앙 배지.
전투할수록 등급↑ = 코어 루프 강화(돈이 아니라 전적이 화폐). listMyFleets/getFleetDetail에
account_kills 서브쿼리 추가. 페이로드 검증, 스크립트 11/0, fresh 체인 완주.

## 2026-06-03 v7.379 — 함대 목록 카드 정체성 + detail 쿼리 코스메틱 정합

함대 목록 카드에 리버리 색 점·길드 엠블럼·누적 킬(⊹N) 표시. getFleetDetail(/:id) 쿼리에도
guild_emblem/guild_tag 추가(f.*엔 accent_color만 있어 길드 배지가 리로드 시 누락되던 것 보강).
이제 목록·상세·프리뷰 전부에서 함대 정체성(리버리/길드/전적) 일관 표시. 스크립트 11/0.

## 2026-06-03 v7.378 — 함대 코스메틱: 길드 엠블럼 소속 배지

함대 프리뷰 보드에 소속 길드 엠블럼/태그 배지 표시(소속 과시). guilds.emblem_emoji/tag는 이미
존재 — fleet 목록 쿼리에 상관 서브쿼리로 guild_emblem/guild_tag 추가(GROUP BY 무변경). 리버리
색이 있으면 배지 테두리도 강조색. 백엔드 페이로드 검증.

## 2026-06-03 v7.377 — 함대 코스메틱: 킬마크/네임플레이트 + 함대 리버리(GP 싱크)

함대 코스메틱 로드맵 2~3단계(전적·소속 축, full-loss 모순 회피):
- 킬마크 데칼: ships.total_kills를 함선에 자동 표식(무료). 5킬↑ 금색 에이스. 프론트 0-backend.
- 기함 네임플레이트: 기함 함명 노출.
- 함대 리버리: fleets.accent_color(mig308) 강조색을 엔진 글로우로 표시 → 멀리서도 함대 식별.
  함대 귀속(함선 격침 무관). 팔레트 10색 제한(임의 hex 차단), 변경 시 200 GP 소각(sink).
  POST /api/fleets/:id/livery (JWT+소유권+가드차감). Fleet Command에 색 스와치 피커.

## 2026-06-03 v7.376 — 함대 미리보기 size_class별 실제 함선 크기

함대 지휘 진형 미리보기에서 모든 함선이 동일 크기로 보이던 것을, size_class별 스케일로 차등화
(프리깃 28px ~ 타이탄 122px). 기함은 고정크기 대신 자기 함급 크기 × 강조 스케일(1.22)+금색 글로우
→ 타이탄 기함은 거대, 프리깃 기함은 작지만 빛남. 순수 CSS(.sc-* 클래스)+렌더 1줄, 전투엔진 무영향.
(함대 코스메틱 로드맵 1단계 — 함선이 커져 향후 기함 꾸미기 여지 확보.)
...
설정으로 통일. 하드코딩 2~4% 제거 → 하우스 우위 강화 + 어드민 튜닝 가능. EV 수치검증.
...
- 결과(손상비례→캡): cruiser(재건4500) 30%=900/60%=1800/캡2700, battleship(11000) 3780/6600캡, titan(52000) 15750/31200캡. frigate/destroyer는 캡 일찍 걸려 준-소모품.
- 대형함은 수리 vs 재건 실제 선택지가 생기고, 빈번한 전투에 반복 GP 싱크 → EVE식 디플레 강화(함대전이 돌면 인플레 없음 논의의 실현).
- 죽은 설정 ship_repair_iron_per_hp(0.5, 코드 미사용) 제거. 광물 싱크는 ship_repair_iron_per_10hp(0.2) 유지.
- 런타임 getSetting 0.03 확인.
...
- 디자인: 반투명 글래스(backdrop-blur) 카드 + 화성 오렌지 외곽 글로우, 상단에 흐르는 gold→mars→red 그라데이션 바, 깜빡이는 ● 오픈베타 뱃지, gold 강조 키워드, 오렌지→레드 그라데이션 CTA. 기존 `--mars(#FF7840)` 테마와 정합.
- 내용 4줄: 실험 단계(예고 없는 오류/변경/점검), 데이터 초기화 가능, 게임 재화(GP·PP) 현금가치·환불 없음, 버그는 🐛로 신고.
- 4개국어(ko/en/ja/zh) — 기존 `tl()` 헬퍼 사용. `LANG`에 따라 렌더.
- 동작: `localStorage 'pw_beta_notice_v1'` 1회성 게이트. DOMContentLoaded +400ms에 랜딩보다 먼저 노출, `acceptBetaNotice()`가 플래그 저장 후 닫음.
- 검증(라이브 미리보기): 데스크탑/모바일(375px) 레이아웃 정상, ko/ja i18n 정상, 수락→플래그 저장→재호출 시 미노출(1회성) 확인. ASSET_VER 7346→7351.
- 후속(선택): 하단 상시 "OPEN BETA" 배지는 별도 작업으로 분리.
...
## 2026-06-09 v7.413 — 함대전 결과 리포트 전술 분석 강화

- 전투 리포트가 함선 role/함급 분포와 전투 이벤트를 합산해 패인 분석을 생성하도록 개선했다.
- 분석 항목에 전자전 피격, 기함 격침, 대형함 저격/폭격 카운터, 소형 러시 카운터, 지원함 격차, 손실률/데미지 격차를 반영했다.
- `/api/battles/:id/report` 응답에 `analysis_items`, `recommendation_items`, `role_counts`, `size_counts`, `event_stats`를 추가했다.
- 전투 결과 모달이 새 분석 항목을 여러 줄로 표시하고, 패배/무승부 시 개선 추천을 노출한다.
