## 2026-07-16 — i18n 이중인코딩 + SEA 배포 누락 감사 반영 (v7.472~473)

### ✅ 수정 완료
- **[MED] i18n.js 이중인코딩 대량(브라우저 실플레이 QA 발견)**: 라운드트립 복구 2,395→0(정상 베트남어 8건 보존). i18n-sea 는 repair 명령(이모지 이식)으로 870건 + 신규 15문자열. SEA 100%.
- **[HIGH] i18n-sea.js 커밋 누락(gitignore assets/* 풋건)**: v7.446 부터 프로덕션 404→SEA 영어 폴백. .gitignore `!assets/*.js`/`!assets/*.css` 영구 예외 + 최초 커밋. 누락 소스 전수 스캔 1건뿐.
- **[게임성] 하이잭 점령 연출(v7.472)**: hijackCaptureFx — 플라이투+충격파+배너+SFX, auto_win/전투승리 2경로. 브라우저 DOM 실증.

### 검증 완료
- node --check 전수, SEA check 2,398/2,398 exit0, 브라우저 육안(한국어 UI/이모지/가운뎃점), Đã 오스트립 0·C1 0. SW 캐시 함정 재현→fix 후 재범프(?v=7485/mars-v141).
...
### ✅ 수정 완료
- **[MED] 절차 합성 BGM 전자음 품질 불만(v7.468)**: OpenGameArt CC0 6트랙(ambient/dark/battle/boss/victory/sad) 다운로드→mp3 교체. _campaignBgmSrc 무드 별칭으로 40 씬이름→6파일 매핑(중복 0, 연속성 src 기준).

### 남은 정리 범위
- 개별 무드 트랙은 같은 파일명으로 교체 가능. tools/gen-bgm.js(절차 폴백) 미사용 유지. 라인별 음성은 보류(v7.467 인프라 유지).

### 검증 완료
- node --check, 별칭 하니스(40→6 매핑 누락 0·연속성·엣지), 6트랙 ffprobe/volumedetect(실음악), 서버 서빙 200 audio/mpeg. 전부 CC0 라이선스.

## 2026-07-02 — 캠페인 BGM 40트랙 실제 생성 감사 반영 (v7.468)

### ✅ 수정 완료
- **[MED] BGM 인프라만 있고 음원 0 → 무음(v7.466 잔여)**: tools/gen-bgm.js 로 40트랙 절차 합성(이음매 없는 루프, 무드별). assets/audio/bgm/*.mp3 (24s/mono/64k, 7.5MB) 채움. 배포 시 캠페인에서 즉시 재생.

### 남은 정리 범위
- 절차 생성 = 기능적 분위기 베드(프로 OST 아님). 같은 파일명으로 프로 트랙 교체 가능. 라인별 음성(영어)은 유저 TTS 제작 대기(v7.467 파이프라인).

### 검증 완료
- 전 40트랙 ffprobe(24s/mono)·volumedetect(무음 아님·비클리핑)·길이≥20s. node --check tools/gen-bgm.js. git 추적 확인.
...
### ✅ 수정 완료
- **[LOW] 서지 배율이 채굴엔 적용되나(v7.436) 수확 결과에 미표시**: 리텐션 훅의 보상 체감 누락. harvest-all/per-claim/globe 수확 토스트에 `⚡ 서지 ×N` 표시 추가(4개국어+SEA). 서버 계약만 소비, 계산 무변경.

### 검증 완료
- `node --check` territory-ui/world-globe/i18n-sea/app-config/sw PASS. SEA 도구 실전(신규 문자열 감지→재생성→커버 100%). `git diff --check`.
