...
| 15 | 처음 로딩 시 모든 high-tier 섹터가 entry-blocked 로 표시, BASE 누르면 정상 | sector overlay 가 `profileLevel` DOM 에서 user level 읽음. BASE 모달 안 열면 DOM 이 default '1' → 모든 entryMinLevel>1 섹터 잠금. | auto-login `/api/auth/me` 성공 시점에 `/api/user/:wallet/base` 미리 fetch → profileLevel 채움 + 텍스처 재합성 | (이번 커밋) |
