...
> 최종 업데이트: 2026-04-29 v5.24 (FSP Campaign Ch4 MVP: Diplomacy / Sandstone Junction) | 이 파일을 먼저 읽으면 코드베이스를 즉시 파악할 수 있습니다.
...
│   ├── migrations/         ← SQL 파일 001~198 (2026-04-29 기준)
...
- **적용된 마이그레이션**: 001 ~ **198** (2026-04-29 기준)
- **총 테이블 수**: 109개+
- **마지막 마이그레이션**: `198_fsp_campaign_ch4_diplomacy.sql`
...
### 현재 구현 상태 (v5.24)
- **MVP 방식**: MCC Campaign Ch1~10과 FSP Campaign Ch1~4는 `server/services/campaign.js`의 서버 결정형 시뮬레이션으로 처리한다. 아직 tactical-lab/v11.1 실시간 전투 엔진에는 연결하지 않았다.
...
> FSP Ch5/Ch6는 전달된 Ch4~6 spec에서 placeholder 상태다. 다음 spec 수령 전까지 구현하지 않는다.
