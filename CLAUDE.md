...
- `server/routes/redemptionRoutes.js`를 추가했다. PP→USDT swap, USDT withdraw, withdraw-all 라우트를 `server/routes/api.js`에서 분리했다.
- 기존 `/api/swap`, `/api/withdraw`, `/api/withdraw-all` URL 계약은 유지한다.
- pending withdrawal 재발급/만료 처리, solvency guard, redemption weekly/daily limit, on-chain liquidity check, signer 호출 흐름은 유지했다.
- 이번 변경은 서버 스파게티 정리 25차다. 다음 후보는 harvest core 또는 claim/hijack core 분리다.
- 검증 기준: `node --check server/routes/redemptionRoutes.js`, `node --check server/routes/api.js`, `node --check server/index.js`, `git diff --check`.
...
- `server/routes/guildRoutes.js`를 추가했다. 길드 생성/가입/초대/권한/채팅/기여/연구/길드전 라우트 전체를 `server/routes/api.js`에서 분리했다.
- 기존 `/api/guild/*` URL 계약과 라우트 순서는 유지한다.
- `guildService`, `seasonService`, `pool`, `getSetting` 의존성은 새 라우터 상단에 명시했다.
- 이번 변경은 서버 스파게티 정리 23차다. 다음 후보는 `api.js` 내 harvest/territory economy core 분리다.
- 검증 기준: `node --check server/routes/guildRoutes.js`, `node --check server/routes/api.js`, `node --check server/index.js`, `git diff --check`.

### v7.460 최신 핸드오프 — Sector Control 라우트 분리

- `server/routes/sectorControlRoutes.js`를 추가했다. 섹터 영향력/컨트롤 조회 `/api/sectors/control`, `/api/sectors/:sectorId/control` 라우트를 `server/routes/api.js`에서 분리했다.
- URL 계약은 유지한다. 기존 섹터 컨트롤 요약/상세 호출은 동일하게 동작한다.
- pixel area, upgrade score, 최근 harvest score, guild id 매핑, influence tier 응답 정책은 유지했다.
- 이번 변경은 서버 스파게티 정리 22차다. 다음 후보는 `api.js` 내 harvest/territory core 또는 guild 계열 분리다.
- 검증 기준: `node --check server/routes/sectorControlRoutes.js`, `node --check server/routes/api.js`, `node --check server/index.js`, `git diff --check`.
...
- `server/routes/questRoutes.js`를 추가했다. 랜덤 퀘스트 조회/진행/수령/트래킹 `/api/quests*` 라우트를 `server/routes/api.js`에서 분리했다.
- URL 계약은 유지한다. 기존 `/api/quests`, `/api/quests/:id/progress`, `/api/quests/:id/claim`, `/api/quests/track` 호출은 동일하게 동작한다.
- 퀘스트 자동 생성, 만료 처리, GP 보상 환산, 일일 보상 캡, XP 지급 정책은 유지했다.
- `/api/quests/:id/progress` 완료 알림에서 존재하지 않는 `wallet` 변수를 참조하던 오류를 인증 wallet `w` 기준으로 수정했다.
- 이번 변경은 서버 스파게티 정리 20차다. 다음 후보는 `api.js` 내 harvest/territory core 또는 guild 계열 분리다.
- 검증 기준: `node --check server/routes/questRoutes.js`, `node --check server/routes/api.js`, `node --check server/index.js`, `git diff --check`.

### v7.457 최신 핸드오프 — Territory Cosmetic 라우트 분리

- `server/routes/cosmeticRoutes.js`를 추가했다. 영토 코스메틱 장착/해제/조회 `/api/cosmetic/*` 라우트를 `server/routes/api.js`에서 분리했다.
- URL 계약은 유지한다. 기존 `/api/cosmetic/equip`, `/api/cosmetic/unequip`, `/api/cosmetic/equipped` 호출은 동일하게 동작한다.
- claim ownership 검증, `user_items` quantity 차감/환수, PP equip fee 차감, `user_cosmetics` upsert, daily/season best-effort side-effect는 유지했다.
- 이번 변경은 서버 스파게티 정리 19차다. 다음 후보는 `api.js` 내 quest 또는 harvest/territory core 계열 분리다.
- 검증 기준: `node --check server/routes/cosmeticRoutes.js`, `node --check server/routes/api.js`, `node --check server/index.js`, `git diff --check`.

### v7.456 최신 핸드오프 — Player Status/Feed 라우트 분리

- `server/routes/playerStatusRoutes.js`를 추가했다. 업적 조회, 공개 뉴스, 온보딩 상태/닫기, 글로벌 activity feed를 `server/routes/api.js`에서 분리했다.
- URL 계약은 유지한다. 기존 `/api/achievements`, `/api/news`, `/api/onboarding/status`, `/api/onboarding/dismiss`, `/api/activity/feed` 호출은 동일하게 동작한다.
- activity feed의 5초 in-memory cache, source별 best-effort query fallback, 온보딩 `requireAuth`/JWT wallet 정책은 유지했다.
- `api.js`의 `achSvc`/`newsSvc` 로드는 claim/transfer side-effect용으로 남겨뒀다.
- 이번 변경은 서버 스파게티 정리 18차다. 다음 후보는 `api.js` 내 quest/cosmetic 또는 harvest/territory core 계열 분리다.
- 검증 기준: `node --check server/routes/playerStatusRoutes.js`, `node --check server/routes/api.js`, `node --check server/index.js`, `git diff --check`.
...
- `server/routes/userBaseRoutes.js`를 추가했다. BASE 탭 통합 조회 `/api/user/:wallet/base`를 `server/routes/api.js`에서 분리했다.
- URL 계약은 유지한다. 기존 `/api/user/:wallet/base` 호출은 동일하게 동작한다.
- lazy rank recalculation, mining interval/rates, territory summary, harvest availability, rank breakthrough response shape는 유지했다.
- 이번 변경은 서버 스파게티 정리 16차다. 다음 후보는 `api.js` 내 sector query 또는 harvest/territory core 계열 분리다.
- 검증 기준: `node --check server/routes/userBaseRoutes.js`, `node --check server/routes/api.js`, `node --check server/index.js`, `git diff --check`.

### v7.453 최신 핸드오프 — Public Stats/Rank 라우트 분리

- `server/routes/statsRoutes.js`를 추가했다. 공개 리더보드/글로벌 통계/클라이언트 에러 리포트/랭크/돌파 진행 조회를 `server/routes/api.js`에서 분리했다.
- URL 계약은 유지한다. 기존 `/api/leaderboard`, `/api/stats`, `/api/error-report`, `/api/ranks`, `/api/breakthrough/:wallet` 호출은 동일하게 동작한다.
- 픽셀 수 리더보드의 실제 `pixels` 테이블 기준 계산, 클라이언트 에러 필드 truncation, rank breakthrough 표시 정책은 유지했다.
- 이번 변경은 서버 스파게티 정리 15차다. 다음 후보는 `api.js` 내 guild 또는 user/base/sector 계열 분리다.
- 검증 기준: `node --check server/routes/statsRoutes.js`, `node --check server/routes/api.js`, `node --check server/index.js`, `git diff --check`.
