  ...
  mtReadFetch('raid-targets', '/api/transport/raids/targets?limit=30', 10000, true)
    .then(function(d) {
      ...
      rows = rows.map(function(t) {
        ...
        var progress = parseInt(t.progress_pct, 10) || 0;
        var progressWindow = progress >= (parseInt(_tspSettings&&_tspSettings.raidMinProg,10)||20) && progress <= (parseInt(_tspSettings&&_tspSettings.raidMaxProg,10)||90);
        var expectedGp = Math.round(lootEstimate * (successPct / 100));
        var priorityScore = expectedGp + (progressWindow ? 12 : -20);
        return Object.assign({}, t, {
          _raidLootEstimate: lootEstimate,
          _raidSuccessPct: successPct,
          _raidExpectedGp: expectedGp,
          _raidProgressWindow: progressWindow,
          _raidPriorityScore: priorityScore
        });
      }).sort(function(a,b){ return (b._raidPriorityScore||0) - (a._raidPriorityScore||0); });
      list.innerHTML = rows.map(function(t) {
        var prog = parseInt(t.progress_pct)||0;
        var lootEstimate = t._raidLootEstimate || 0;
        var successPct = t._raidSuccessPct || 0;
        var expectedGp = t._raidExpectedGp || 0;
        ...
        var priorityLabel = t._raidExpectedGp >= 80
          ? (LANG==='ko'?'고가치 표적':LANG==='ja'?'高価値標的':LANG==='zh'?'高价值目标':'High-value target')
          : t._raidExpectedGp >= 35
            ? (LANG==='ko'?'실속 표적':LANG==='ja'?'堅実標的':LANG==='zh'?'稳健目标':'Efficient target')
            : (LANG==='ko'?'저수익 표적':LANG==='ja'?'低収益標的':LANG==='zh'?'低收益目标':'Low-yield target');
        var windowLabel = t._raidProgressWindow
          ? (LANG==='ko'?'약탈 가능 구간':LANG==='ja'?'略奪可能帯':LANG==='zh'?'可掠夺窗口':'Raid window')
          : (LANG==='ko'?'타이밍 주의':LANG==='ja'?'タイミング注意':LANG==='zh'?'注意时机':'Watch timing');
        var windowColor = t._raidProgressWindow ? 'var(--gn)' : '#FFB347';
        var bar = '<div style="width:100%;height:4px;background:rgba(255,255,255,.08);border-radius:2px;overflow:hidden;margin-top:4px">'
        ...
        return '<div style="padding:10px;border-radius:7px;border:1px solid rgba(255,107,107,.22);background:rgba(255,107,107,.03);margin-bottom:8px">'
          ...
          + '<span style="color:#FFB347;border:1px solid rgba(255,179,71,.25);border-radius:999px;padding:2px 6px;background:rgba(255,179,71,.06)">'+priorityLabel+'</span>'
          + '<span style="color:'+windowColor+';border:1px solid rgba(255,255,255,.1);border-radius:999px;padding:2px 6px;background:rgba(255,255,255,.035)">'+windowLabel+'</span>'
          + '</div>' + laneContext
          + '</div>';
      }).join('');
    }).catch(function(){});
